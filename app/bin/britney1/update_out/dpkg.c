...
// enlarge this is britney has issues to parse packages
...
static dpkg_package *read_package( FILE *f ) {
	...
	/*
	 * Support Breaks by treating them as Conflicts.
	 */
	if (strcasecmp("Conflicts", e->name) == 0
	    || strcasecmp("Breaks", e->name) == 0) {
	    deplist *pkgs = read_dep_and(e->value);
	    if (result->conflicts == NULL)
		result->conflicts = pkgs;
	    else {
		// TODO(dato): create extend_deplist() in templates.h?
		deplist *end = result->conflicts;
		while (end->next)
		    end = end->next;
		end->next = pkgs;
	    }
...
static int max_instone_len = 0;
static int cur_instone_len = 0;
...
static instonelist *insert_instonelist(instonelist *old, collpackagelist *instone)
{
    ...
    cur_instone_len++;
    if (cur_instone_len > max_instone_len) max_instone_len = cur_instone_len;
...
static int cur_install =0;
static int max_install = 0;
static int cur_conflicted = 0;
static int max_conflicted = 0;
...
static void install(dpkg_packages *pkgs, dpkg_collected_package *cpkg) {
	...
	for (conf = conflicts; conf != NULL; conf = conf->next) {
	    ...
	    if (conf->value->conflicted == 1) {
	    	cur_conflicted++;
	    	if (max_conflicted < cur_conflicted)
			max_conflicted = cur_conflicted;
	    }
	}
	free_collpackagelist(conflicts);
	cur_install++;
	if (max_install < cur_install) max_install = cur_install;
    }
...
static void uninstall(dpkg_packages *pkgs, dpkg_collected_package *cpkg) {
	...
	for (conf = conflicts; conf != NULL; conf = conf->next) {
	    ...
	    conf->value->conflicted--;
	    if (conf->value->conflicted == 0) cur_conflicted--;
	}
	free_collpackagelist(conflicts);
	cur_install--;
    }
...
int checkinstallable(dpkg_packages *pkgs, collpackagelist *instoneof) {
    ...
    unsigned long counter = 500000000;

    int last_max_inst = max_install;
    int last_max_instone = max_instone_len;
    int last_max_conflict = max_conflicted;
    ...
    if (max_install>last_max_inst || max_instone_len>last_max_instone || max_conflicted >last_max_conflict) {
        printf("MAX: i:%d / c:%d / i1:%d %s\n", max_install,max_conflicted,max_instone_len,instoneof->value->pkg->package);
    }
